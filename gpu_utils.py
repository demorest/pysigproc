...
def gpu_dedisp_and_dmt(cand, device=0):
    ...
    dmt_return = cuda.to_device(np.zeros((256, cand.data.shape[0]//time_decimation_factor), dtype=np.float32),stream=stream)
    cand_data_out = cuda.to_device(np.zeros((256, cand.data.shape[0]//time_decimation_factor), dtype=np.float32),stream=stream)
    #cand_data_out = cuda.to_device(np.zeros_like(cand.data.T, dtype=np.uint8))
    ...
    def gpu_dedisp(cand_data_in, chan_freqs, dm, cand_data_out, tsamp, time_decimation_factor):
        ...
        if ii < cand_data_in.shape[0] and jj < cand_data_in.shape[1]:
            disp_time = int(-4148808.0 * dm * (1 / (chan_freqs[0]) ** 2 - 1 / (chan_freqs[ii]) ** 2) / 1000 / tsamp)
            cuda.atomic.add(cand_data_out,(int(ii/16), int(jj/time_decimation_factor)), cand_data_in[ii, (jj + disp_time) % cand_data_in.shape[1]])
    ...
    def gpu_dmt(cand_data_in, chan_freqs, dms, cand_data_out, tsamp, time_decimation_factor):
        ...
        if ii < cand_data_in.shape[0] and jj < cand_data_in.shape[1] and kk < dms.shape[0]:
            disp_time = int(
                -1 * 4148808.0 * dms[kk] * (1 / (chan_freqs[0]) ** 2 - 1 / (chan_freqs[ii]) ** 2) / 1000 / tsamp)
            cuda.atomic.add(cand_data_out, (kk, int(jj/time_decimation_factor)), cand_data_in[ii, (jj + disp_time) % cand_data_in.shape[1]])
    ...
    gpu_dmt[blockspergrid, threadsperblock, stream](cand_data_in, chan_freqs, dm_list, dmt_return, float(cand.tsamp), int(time_decimation_factor))
...
def gpu_dedisp_and_dmt_crop(cand, device=0):
    ...
    chan_freqs = cuda.to_device(np.array(cand.chan_freqs, dtype=np.float32),stream=stream)
    cand_data_in = cuda.to_device(np.array(cand.data.T, dtype=np.uint8),stream=stream)
    dmt_on_device = cuda.device_array((256, cand.data.shape[0]//time_decimation_factor), dtype=np.float32,stream=stream)
    cand_dedispersed_on_device = cuda.device_array((256, cand.data.shape[0]//time_decimation_factor), dtype=np.float32,stream=stream)
    ...
    gpu_dedisp[blockspergrid_2d_in, threadsperblock_2d, stream](cand_data_in, chan_freqs, float(cand.dm), cand_dedispersed_on_device,
                                               float(cand.tsamp), int(time_decimation_factor))
    ...
    crop_time[blockspergrid_2d_out, threadsperblock_2d, stream](cand_dedispersed_on_device,cand_dedispersed_out,int(int(cand_dedispersed_on_device.shape[1]/2)-128))
    ...
    gpu_dmt[blockspergrid, threadsperblock_3d, stream](cand_data_in, chan_freqs, dm_list, dmt_on_device, float(cand.tsamp), int(time_decimation_factor))

    crop_time[blockspergrid_2d_out, threadsperblock_2d, stream](dmt_on_device,dmt_return,int(int(cand_dedispersed_on_device.shape[1]/2)-128))
